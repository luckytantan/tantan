          ...
          $delpass = $_POST['delpass'];
            ...
            $result = $pdo -> query($sql);
               foreach ($result as $row){
                ...
                {$id = $delete;
                  $sql = 'delete from tb9 where id=:id';
...
if (!empty($_POST['number']) && !empty($_POST['name']) && !empty($_POST['comment']) && !empty($_POST['pass']))
   {$name = $_POST['name'];
      ...
      $result = $pdo -> query($sql);
         foreach ($result as $row){
           if($row['id'] == $number)
              ...
              $name = $_POST['name'];
              $comment = $_POST['comment']; 
                   $sql = 'UPDATE tb9 SET name=:name,comment=:comment, date=:date, pass=:pass WHERE id=:id';
                   $stmt = $pdo->prepare($sql);
    ...
    if(!empty($_POST['comment']) && !empty($_POST['pass']))
        {$postedAt = date("Y年m月d日 H:i:s");
        $stmt = $pdo->query($sql);
        $sql = $pdo -> prepare("INSERT INTO tb9 (name, comment, date, pass) VALUES (:name, :comment, :date, :pass)");
